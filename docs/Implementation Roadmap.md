...
- [x] **DOM Writer (Server-side):** Create `DomXmlWriter.java` in the server package. 
    ...
    *   Use `DocumentBuilderFactory` to write a validated match replay `.xml` file.
- [x] **Test:** Run a quick game to completion. Verify that `.xml` is successfully generated in your project root and that it matches the XSD schema.
